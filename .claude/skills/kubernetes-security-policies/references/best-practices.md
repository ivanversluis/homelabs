...
**kube-bench:**
```bash
# Do NOT apply directly from remote URLs.
# Use a pinned ref and validate first.

KUBEBENCH_MANIFEST_URL="https://raw.githubusercontent.com/aquasecurity/kube-bench/<PINNED_REF>/job.yaml"
curl -fsSL "$KUBEBENCH_MANIFEST_URL" -o /tmp/kube-bench-job.yaml

# Review and verify
less /tmp/kube-bench-job.yaml
sha256sum /tmp/kube-bench-job.yaml
